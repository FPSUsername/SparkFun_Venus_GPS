...
void setup()
{
  Serial.begin(38400); // Arduino serial
  gpsSerial.begin(38400); // GPS serial

//  Factory reset
//  byte Factory[] = {160, 161, 00, 02, 04, 01, 05, 13, 10} // Factory reset 			A0 A1 00 02 04 01 05 0D 0A 
...
//  byte baudrateCmd[] = {160, 161, 00, 04, 05, 00, 01, 00, 04, 13, 10}; // 9600		A0 A1 00 04 05 00 01 00 04 0D 0A
...
//  byte waas[] = {160, 161, 00, 03, 55, 01, 00, 54, 13, 10}; // WAAS ON				A0 A1 00 03 37 01 00 36 0D 0A
//  byte waas[] = {160, 161, 00, 03, 55, 00, 00, 55, 13, 10}; // WAAS OFF				A0 A1 00 03 37 00 00 37 0D 0A
//	byte mode[] = {160, 161, 00, 03, 09, 00, 00, 09, 13, 10}; // aucun					A0 A1 00 03 09 00 00 09 0D 0A
...
//  gpsSerial.flush();
//  delay(10);
//  gpsSerial.begin(38400); // New baudrate

//  gpsSerial.write(updateRateCmd, sizeof(updateRateCmd));
//  gpsSerial.write(waas, sizeof(waas));
//  gpsSerial.write(mode, sizeof(mode));
}
